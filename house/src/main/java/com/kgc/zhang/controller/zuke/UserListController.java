...
public class UserListController {
    ...
    public String findhasuserlist(Model model,HttpServletRequest request) {
        user user = (user) request.getSession().getAttribute("user");
        List<userlist> userlist = userlistService.UserListID(user.getId());
        if (userlist.size()==0){
            model.addAttribute("mainPage","../zuke/updateuserlist.jsp");
            return"zuke/main";
        }else {
...
@RequestMapping("/checkuserlist")
    public  String checkuserlist  (userlist userlist,Model model,HttpServletRequest request){
    user user = (user) request.getSession().getAttribute("user");
...
//判断是否完善用户资料;
    ...
    @ResponseBody
    public Map checkuserlists(HttpServletRequest request, Model model) {
        Map map = new HashMap();
        user user = (user) request.getSession().getAttribute("user");
