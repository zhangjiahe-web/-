...
<h1>待缴租金列表</h1>
                ...
                <form action="/house/paid/mytopaidlist.action" method="post" name="myform">
					...
					<div style="LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right; margin-top: 10px" class="page-div">
						...
						<a href="/mytopaidlist?pageNumStr=${pageInfo.firstPage}">首页</a>
						<c:if test="${pageInfo.hasPreviousPage}">
							<a href="/mytopaidlist?pageNumStr=${pageInfo.prePage}">上一页</a>
						</c:if>
						<c:forEach items="${pageInfo.navigatepageNums}" var="i">
							<a href="/mytopaidlist?pageNumStr=${i}">${i}</a>
						</c:forEach>
						<c:if test="${pageInfo.hasNextPage}">
							<a href="/mytopaidlist?pageNumStr=${pageInfo.nextPage}">下一页</a>
						</c:if>
						<a href="/mytopaidlist?pageNumStr=${pageInfo.lastPage}">尾页</a>
					</div>
							</span>
