		...
		<form id="houseForm" name="houseForm"
						...
						<c:forEach items="${userlist.list}" var="apply">
							<tr
								...
								<td>${apply.userlist.phone}</td>
									...
									<td>${apply.address}</td>
									<td>${apply.price}</td>
									<td><c:if test="${apply.status!=null}">
										申请中
									</c:if></td>
							</tr>
						</c:forEach>
			...
			<tr>
               ...
               <div style="LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right; margin-top: 10px" class="page-div">
                   ...
                   <a href="/getmyapply?pageNumStr=${userlist.firstPage}">首页</a>
            <c:if test="${userlist.hasPreviousPage}">
				<a href="/getmyapply?pageNumStr=${userlist.prePage}">上一页</a>
			</c:if>
            <c:forEach items="${userlist.navigatepageNums}" var="i">
				<a href="/getmyapply?pageNumStr=${i}">${i}</a>
			</c:forEach>
            <c:if test="${userlist.hasNextPage}">
				<a href="/getmyapply?pageNumStr=${userlist.nextPage}">下一页</a>
			</c:if>
            <a href="/getmyapply?pageNumStr=${userlist.lastPage}">尾页</a>
               </div>
           </span>
